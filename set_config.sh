...
function changeConfigFile() {
	...
	if [[ -f "network-config-aws.json" ]]; then
		rm network-config-aws.json
	fi
	cp network-config-aws-template.json network-config-aws.json
	sed -i "s/ORG_NAME/$ORG_NAME/g" network-config-aws.json
